        ...
        >
          {visibleMessages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`flex w-full animate-fade-in ${myMsg ? 'justify-end' : 'justify-start'}`}
              >
                <div
                  className={`max-w-[75%] rounded-lg px-3 py-1.5 text-sm shadow relative break-words whitespace-pre-wrap ${
                    myMsg 
                ...
                >
                  ...
                  <div className="absolute bottom-1 right-2 flex items-center gap-0.5 text-[10px] text-[#8696a0]">
                    ...
                    {myMsg && settings.showReadTicks && (
                      <span className="text-[#53bdeb] text-xs font-bold">✓✓</span>
                    )}
                  </div>
                </div>
              </div>
